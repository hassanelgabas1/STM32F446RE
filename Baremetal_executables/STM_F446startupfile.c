...
{  
	...
	uint32_t Size = (uint32_t) &_edata - (uint32_t)&_sdata ; /*bec. _edata is pointer */
	uint8_t* SrcPtr = (uint8_t*) &_etext ; /*to cast it as ponter to uint8_t*/
	uint8_t* Destptr = (uint8_t*) &_sdata ;
	uint32_t i ;
		for (i =0 ; i<Size ; i++ )
		{
			*Destptr = *SrcPtr ;  /*equivalent to *Destptr++ = *SrcPtr++  */
			Destptr++ ; 
			SrcPtr ++ ;
		}
	/*Initialising bss section  */
